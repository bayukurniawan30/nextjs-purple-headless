...
import { FieldSchema, Collection } from '@/type/api'
import { useFieldsStore } from '@/hooks/availableFields'

interface FormCollectionData {
  name: string
  status: string
  // fields: any
}
...
const EditCollectionPage = ({ params }: { params: { id: string } }) => {
  ...
  const PageMeta: PageMeta = {
    ...
    breadcrumb: [
      ...
      {
        text: 'Edit',
        href: `/collections/edit/${params.id}`,
      },
    ],
  ...
  const { data, error, isLoading } = useSWR<Collection>(`/collections/${params.id}`, () =>
    axios
  ...
  const onSubmitHandler = async (values: FormCollectionData) => {
    try {
      setDisable(true)
      ...
      axios
        ...
        .then((res) => {
          if (res.status === 200) {
            enqueueSnackbar(`Collection has been updated successfully`, {
              variant: 'success',
            ...
            setDisable(false)
            enqueueSnackbar(`Failed to update existng collection. Please try again`, {
              variant: 'error',
              anchorOrigin: { horizontal: 'right', vertical: 'bottom' },
            })
          }
